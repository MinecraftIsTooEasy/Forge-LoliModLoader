...
public class ClasspathModCandidateFinder implements ModCandidateFinder {
	...
	private static Map<Path, List<Path>> getPathGroups() {
		...
		Set<Path> cp = new HashSet<>(Launch.knotLoader.getCodeSource());
	...
	public void findCandidates(ModCandidateConsumer out) {
		if (FishModLoader.isDevelopmentEnvironment()) {
			...
			try {
				Enumeration<URL> mods = Launch.knotLoader.getClassLoader().getResources("loli.mod.json");

				while (mods.hasMoreElements()) {
					URL url = mods.nextElement();

					try {
						Path path = LoaderUtil.normalizeExistingPath(UrlUtil.getCodeSource(url, "loli.mod.json"));
