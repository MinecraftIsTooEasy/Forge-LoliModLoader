...
public final class ModDiscoverer {
	...
	private final EnvType envType = FishModLoader.getEnvironmentType();
	...
	public List<ModCandidate> discoverMods(Map<String, Set<ModCandidate>> envDisabledModsOut) throws ModResolutionException {
		...
		// add builtin mods
		for (ModCandidate.BuiltinMod mod : FishModLoader.getBuiltinMods()) {
			ModCandidate candidate = ModCandidate.createBuiltin(mod, versionOverrides, depOverrides);
			candidates.add(MetadataVerifier.verifyIndev(candidate, FishModLoader.isDevelopmentEnvironment()));
		}

		// Add the current Java version
		candidates.add(MetadataVerifier.verifyIndev(createJavaMod(), FishModLoader.isDevelopmentEnvironment()));
	...
	private ModCandidate createJavaMod() {
		BuiltinModMetadata metadata = new BuiltinModMetadata.Builder("java", System.getProperty("java.specification.version").replaceFirst("^1\\.", ""))
				.setName(System.getProperty("java.vm.name"))
				.build();
		ModCandidate.BuiltinMod builtinMod = new ModCandidate.BuiltinMod(Collections.singletonList(Paths.get(System.getProperty("java.home"))), metadata);
	...
	final class ModScanTask extends RecursiveTask<ModCandidate> {
		...
		private LoaderModMetadata parseMetadata(InputStream is, String localPath) throws ParseMetadataException {
			return ModMetadataParser.parseMetadata(is, localPath, parentPaths, versionOverrides, depOverrides, FishModLoader.isDevelopmentEnvironment());
		}
	}
}
